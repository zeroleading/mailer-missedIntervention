...
function getPreviewData() {
  try {
    ...
    // 1. Get Student Name from active row (Forced as String)
    const studentName = String(sheet.getRange(activeRow, CONFIG.COL_STUDENT_NAME).getValue() || "[Student Name]");
    ...
    if (lastColumn > CONFIG.COL_STAR) {
      const sessionRecent = String(sheet.getRange(1, lastColumn - 1).getValue());
    ...
    throw new Error(error.message); // Pass error explicitly to the frontend
  }
}